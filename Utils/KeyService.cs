...
namespace BuildBackup
{
    class KeyService
    {
        private static Dictionary<ulong, byte[]> keys = new Dictionary<ulong, byte[]>()
        {
            ...
            // [0xA98C7594F55C02F0] = "????????????????????????????????".ToByteArray(), // TactKeyID 252
            [0x259EE68CD9E76DBA] = "465D784F1019661CCF417FE466801283".ToByteArray(), // TactKeyID 253
            // [0x6A026290FBDB3754] = "????????????????????????????????".ToByteArray(), // TactKeyID 255
            ...
            [0xC8753773ADF1174C] = "1E0E37D42EE5CE5E8067F0394B0905F2".ToByteArray(), // TactKeyID 262
            // [0x83A2AB72DD8AE992] = "????????????????????????????????".ToByteArray(), // TactKeyID 263
            // [0xBEAF567CC45362F0] = "????????????????????????????????".ToByteArray(), // TactKeyID 264
            // [0x7BB3A77FD8D14783] = "????????????????????????????????".ToByteArray(), // TactKeyID 265
            // [0x8F4098E2470FE0C8] = "????????????????????????????????".ToByteArray(), // TactKeyID 266
            ...
            // [0x814E1AB43F3F9345] = "????????????????????????????????".ToByteArray(), // TactKeyID 276
            // [0x1FBE97A317FFBEFA] = "????????????????????????????????".ToByteArray(), // TactKeyID 277
            ...
            [0x2C547F26A2613E01] = "37C50C102D4C9E3A5AC069F072B1417D".ToByteArray(), // BNA 1.5.0 Alpha
        };
