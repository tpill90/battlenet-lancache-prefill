...
namespace BattleNetPrefill.Handlers
{
    public class EncodingFileHandler
    {
        ...
        private EncodingFile GetEncoding(BuildConfigFile buildConfig, bool parseTableB = false, bool checkStuff = false)
        {
            ...
            byte[] content = _cdn.GetRequestAsBytesAsync(RootFolder.data, buildConfig.encoding[1]).Result;

            if (encodingSize != 0 && encodingSize != content.Length)
            {
                content = _cdn.GetRequestAsBytesAsync(RootFolder.data, buildConfig.encoding[1]).Result;
